...
def SSI_COV_iterator(yk, i, nmin, nmax, incr=2, plot=False):
    ...
    λ = np.zeros((n.shape[0],nmax),dtype=np.complex_)
    ...
    for ii, no in np.ndenumerate(n):
        λ[ii,:no], FN[ii,:no], ZT[ii,:no], VV[ii,:,:no] = SSI_COV(T,no) 
...
def SSI_DATA_iterator(yk, i, nmin, nmax, incr=2, plot=False):
    ...
    for ii, no in np.ndenumerate(n):
        ...
        λ[ii,:no],FN[ii,:no],ZT[ii,:no],VV[ii,:,:no] = SSI_DATA(Pi,Pi1,Yii,no) 
...
def stabilization_diagram(FN, ZT, VV, title, 
    ...
    if plot:
        ...
        plt.legend([ko, go, ro], 
                  ["New pole", 
                   "Stable frequency",
                   "Stable frequency and damping"], prop=g_for)
